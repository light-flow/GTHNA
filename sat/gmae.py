...
from torch_geometric.utils.dropout import dropout_edge, dropout_path
from torch_geometric.utils import add_self_loops, remove_self_loops
from torch_geometric.utils import to_dense_adj, dense_to_sparse

import sys
from pygod.pygod.models import GCN

from drop_graph import dropout_subgraph
...
def setup_module(m_type, enc_dec, in_dim, num_hidden, out_dim, num_layers, dropout, activation, residual, norm, nhead,
                 nhead_out, attn_drop, negative_slope=0.2, concat_out=True, aggr='sum') -> nn.Module:
    if m_type == "gat":
        mod = GAT(
            in_dim=in_dim,
    ...
    def mask_attr_prediction(self, x, edge_index):
        ...
        if _drop_edge_rate > 0:
            ...
            if self.select_gano_num:
                ...
                for j in range(self.select_gano_num):
                    ...
                    a_ano, s_ano = compute_G_ano(to_dense_adj(use_mask_edge_edge_index, max_num_nodes=num_nodes)[0],
                                                 use_x)
                    G_ano = a_ano * self.aano_weight + s_ano * self.sano_weight
                    # print('G_ano',G_ano)
                    if G_ano < G_ano_init:
        ...
        if _drop_path_rate > 0:
            if self.select_gano_num:
                G_ano_init = float('inf')
                for j in range(self.select_gano_num):
                    use_mask_path_edge_index, masked_path_edges, _ = dropout_subgraph(edge_index, p=_drop_path_rate,
                                                                                      walk_length=self.drop_path_length,
                    ...
                    a_ano, s_ano = compute_G_ano(to_dense_adj(use_mask_path_edge_index, max_num_nodes=num_nodes)[0],
                                                 use_x)
                    G_ano = a_ano * self.aano_weight + s_ano * self.sano_weight
                    # print('G_ano',G_ano)
                    if G_ano < G_ano_init:
        ...
        if _drop_edge_rate > 0 or _drop_path_rate > 0:
            ...
            if self.predict_all_edge:
                ...
                s_rec = torch.where((((use_s == 0) & (s_init == 1)) | (self.neg_s).to(use_s.device)), struct_recon,
                                    s_init)
            else:
