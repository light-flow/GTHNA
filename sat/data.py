...
import numpy as np
import os
from .position_encoding import LapEncoding
...
class GraphDataset(object):
    def __init__(self, dataset, abs_pe_dim, degree=True, k_hop=2, se="gnn", use_subgraph_edge_attr=False,
                 cache_path=None, return_complete_index=True):
        self.dataset = dataset
        self.n_features = dataset[0].x.shape[-1]
        self.degree = degree
        self.compute_degree()
        self.abs_pe_list = [LapEncoding(abs_pe_dim).compute_pe(dataset[0])]
...
class EmbeddingSet(torch.utils.data.Dataset):
    def __init__(self, nodes_embedding):
        self.nodes = nodes_embedding
