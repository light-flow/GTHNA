...
def build_args():
    ...
    parser.add_argument("--msize", type=int, default=512)
...
def dgl_to_adj_tensor(graph):
    """
    将 DGL 图转换为邻接矩阵张量。

    参数：
        graph (dgl.DGLGraph): 输入的 DGL 图。

    返回：
        torch.Tensor: 形状为 (n, n) 的邻接矩阵张量。
    """
    # 将 DGL 图转换为邻接矩阵（稀疏格式）

    adjacency_matrix = dgl.khop_adj(graph, 1)

    return adjacency_matrix.cuda()